...
class RayPPOTrainer:
        ...
        for data_source, var2metric2val in data_src2var2metric2val.items():
            ...
            for var_name, metric2val in var2metric2val.items():
                ...
                for metric_name, metric_val in metric2val.items():
                    if (var_name == core_var) and any(metric_name.startswith(pfx) for pfx in ["mean", "maj", "best"]) and (metric_name.endswith("@1")):
                        metric_sec = "val-core"
                    else:
                        metric_sec = "val-aux"
    ...
    def fit(self):
        ...
        is_swap = self.config.trainer.is_swap
        swap_batch_num = self.config.trainer.swap_batch_num
        for epoch in range(self.config.trainer.total_epochs):
            for batch_dict in self.train_dataloader:
                ...
                if is_swap:
                    noisy_batchs = []
                    for i in range(swap_batch_num):
                        noisy_batch, decay = swap_image_in_batch(
                            batch,
                            self.processor,
                            self.config.data,
                            ...
                            decay_mode=self.config.trainer.decay_mode,
                            decay_coef=self.config.trainer.decay_coef,
                            warmup_ratio=self.config.trainer.decay_warmup_ratio,
                        )
                        noisy_batchs.append(noisy_batch)
                    noisy_batchs = DataProto.concat(noisy_batchs)
                    metrics["training/noisy_decay"] = decay
                # pop those keys for generation
                if "multi_modal_inputs" in batch.non_tensor_batch.keys():
                    gen_batch = batch.pop(
                        batch_keys=["input_ids", "attention_mask", "position_ids"],
                        non_tensor_batch_keys=["raw_prompt_ids", "multi_modal_data", "multi_modal_inputs", "reward_model"],
                    )
                    if is_swap:
                        noisy_gen_batch = noisy_batchs.pop(
                            batch_keys=["input_ids", "attention_mask", "position_ids"],
                            non_tensor_batch_keys=["raw_prompt_ids", "multi_modal_data", "multi_modal_inputs", "reward_model"],
                        )
                else:
                ...
                with _timer("step", timing_raw):
                    ...
                    with _timer("gen", timing_raw):
                        if is_swap:
                            ...
                            gen_batch = gen_batch.interleave_batches(1 + swap_batch_num)
                        if not self.async_rollout_mode:
                        ...
                        gen_batch_output.non_tensor_batch["image_status"] = \
                            np.array(["normal" if (i // self.config.actor_rollout_ref.rollout.n) % 2 == 0 
                    ...
                    repeat = (1 + swap_batch_num) * self.config.actor_rollout_ref.rollout.n if is_swap else self.config.actor_rollout_ref.rollout.n
